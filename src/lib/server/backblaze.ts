...
/**
 * Resolve file ids (e.g. sources.avatar, pending_edits.avatar) to signed
 * URLs, in one batched query.
 */
export async function getAvatarUrlsByFileIds(
	...
	const records = await db.select().from(files).where(inArray(files.id, ids));
	await Promise.all(
